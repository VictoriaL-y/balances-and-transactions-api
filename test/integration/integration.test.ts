...
describe("Balances, Transactions and Historical balances API", () => {

  describe("Try to use an invalid route", () => {
    it("Should recieve an error message", async () => {
  ...
  describe("GET /historical-balances", () => {
    it("Should recieve an object with a proper array. Inside should be the daily balances within a specific date range", async () => {
      let response = await request(app).get("/historical-balances?from=2022-06-28&to=2022-06-30&sort=asc");

      expect(response.status).toBe(200);
      expect(response.body).toEqual({
        "balances": [{ "date": "28/06/2022", "amount": 10234, "currency": "EUR" },
      ...
      expect(response.status).toBe(200);
      expect(response.body).toEqual({
        "balances": [{ "date": "05/01/2022", "amount": 1514, "currency": "EUR" },
      ...
      expect(response.status).toBe(200);
      expect(response.body).toEqual({ "balances": [{ "date": "03/05/2022", "amount": 14245, "currency": "EUR" }] });
